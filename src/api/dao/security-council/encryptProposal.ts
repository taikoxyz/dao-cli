 ...
 */
async function getPublicKeyForMember(address: Address, config: INetworkConfig): Promise<`0x${string}` | null> {
  try {
...
export async function encryptEmergencyProposal(
  ...
  const actionBytes = encodeAbiParameters(parseAbiParameters('(address to, uint256 value, bytes data)[]'), [
    actions.map((a) => ({ to: a.to, value: a.value, data: a.data })),
  ]);

  // Encrypt the proposal
  const { encrypted, symmetricKey } = encryptProposal(JSON.stringify(metadata), hexToUint8Array(actionBytes));
