 ...
 */
export default async function getDelegates(
                ...
                if (contentUrl && contentUrl.startsWith('ipfs://')) {
                    try {
                        ...
                        if (metadata && typeof metadata === 'object') {
                            profile.identifier = (metadata as any).identifier || (metadata as any).name || (metadata as any).title || (metadata as any).displayName;
 ...
 */
export async function getDelegateCount(
    config: INetworkConfig
): Promise<number> {
    try {
        const client = getPublicClient(config);
